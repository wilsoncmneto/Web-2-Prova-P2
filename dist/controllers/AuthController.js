...
const { normalizeEmail, normalizeText, sanitizeUser, schemas, sendValidationError, validateYupSchema, } = require('../utils/validation');
const router = Router();
function buildRecoverPasswordUrl(baseUrl, email, recoverPassword) {
    const separator = baseUrl.includes('?') ? '&' : '?';
    return `${baseUrl}${separator}email=${encodeURIComponent(email)}&key=${encodeURIComponent(recoverPassword)}`;
}
